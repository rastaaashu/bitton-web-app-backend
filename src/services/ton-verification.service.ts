...
export interface TonBalanceResult {
  verified: boolean;
  balance: string; // Balance in nanotons (raw)
...
async function tonApiRequest(method: string, params: Record<string, string>): Promise<any> {
 ...
 */
export async function verifyTonBalance(
  tonAddress: string,
  ...
  if (!BTN_JETTON_MASTER) {
    logger.warn("TON_BTN_JETTON_MASTER not configured — using snapshot only");
    if (snapshotBalance) {
      return {
        verified: true,
  ...
  try {
    ...
    if (jettonBalance === null) {
      ...
      if (snapshotBalance && BigInt(snapshotBalance) > 0n) {
        logger.warn(
          `Snapshot shows balance ${snapshotBalance} but no on-chain jetton wallet found for ${tonAddress}`
        );
    ...
    if (snapshotBalance) {
      ...
      if (diff > tolerance && snapshotBig > 0n) {
        logger.warn(
          `Balance mismatch for ${tonAddress}: on-chain=${jettonBalance}, snapshot=${snapshotBalance}`
        );
      }
    ...
    logger.error(`TON balance verification failed for ${tonAddress}:`, err);
...
async function getJettonBalance(
  ...
  try {
    ...
    const wallet = wallets[0] as JettonWalletInfo;
    ...
    try {
      ...
      return (wallets[0] as JettonWalletInfo).balance || "0";
    } catch {
...
export async function batchVerifyBalances(
  entries: Array<{ tonAddress: string; snapshotBalance: string }>
): Promise<Array<TonBalanceResult & { tonAddress: string }>> {
  const results: Array<TonBalanceResult & { tonAddress: string }> = [];
...
function formatJettonBalance(rawBalance: string): string {
  try {
...
export function validateSnapshot(
  entries: Array<{ tonAddress: string; balanceBtn: string }>
): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  const seen = new Set<string>();
  let total = 0n;
  const MAX_SUPPLY = 21_000_000_000_000n; // 21M with 6 decimals
  ...
  if (total > MAX_SUPPLY) {
    errors.push(
      `Total snapshot balance (${formatJettonBalance(total.toString())}) exceeds max supply (21M BTN)`
    );
  }
